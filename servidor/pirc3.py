...
class DynamoDBService:
    def __init__(self, region='us-east-1', server_id='server_local'):
        # NOTE: boto3 will automatically use IAM Role credentials if available
    ...
    # --- DynamoDB Implementation (DynamoDBService class) ---
    ...
    def is_nick_online(self, nick: str) -> bool:
        ...
        try:
            response = self.users_table.get_item(
                Key={'Nick': nick},
                ProjectionExpression='#s', # Only fetch the status attribute
            ...
            # If the DB check fails, assume it's in use to prevent conflicts.
            return True
    ...
    def quit_user(self, client: ClientRegistration):
        try:
            if client.nick.startswith("guest") and not client.nick_set:
                #log.warning(f"DB-WRITE BLOCKED: Refusing to register temporary nick: {client.nick}")
                return
    ...
    def get_all_channels(self) -> List[Dict]:
        ...
        """
        try:
            # Note: Scanning is efficient for small channel lists but should be reviewed
            # for performance if you anticipate thousands of channels.
            response = self.channels_table.scan(
                # Fetch Name, Topic, and Members list
            ...
            for item in response.get('Items', []):
                # Calculate the member count from the size of the Members set
                member_count = len(item.get('Members', set()))
                
                # Only list channels that have members
                if member_count > 0:
    ...
    def send_message(self, target: str, sender_nick: str, content: str):
        ...
        self.msgs_table.put_item(Item={
            'Target': target,            # Partition Key (Channel name OR Nickname)
...
class MessagePoller(threading.Thread):
    ...
    def run(self):
        ...
        while self.running:
            # 1. Identify what we need to poll for:
            #    A. All active channels on this server
            #    B. All active USERS on this server (for Private Messages)
            poll_targets = set()
            
            with self.server.users_lock:
                # Add Local Users (Active Nicks)
                for nick in self.server.users.keys():
                    poll_targets.add(nick)
                
                # Add Channels local users are in
            ...
            # 2. Poll DB for each target (Channel or Nick)
            for target in poll_targets:
                messages = self.db.fetch_new_messages(target, self.last_check)
                for msg in messages:
                    # Ignore messages sent by THIS server (we already displayed them locally)
...
class PircServer:
    def __init__(self, host="0.0.0.0", port=6667, db_region='us-east-1'):
        ...
        self.clients = {} # {fileno: ClientRegistration}
        self.users = {}   # {nick: ClientRegistration}
    ...
    def send_reply(self, client: ClientRegistration, numeric_code: str, text: str, target: str = None):
        ...
        # Use the client's current nick as the target if none is specified
        target = target if target else client.nick
        
        # Standard IRC format: :<servername> <numeric_code> <target> <text>
    ...
    def broadcast_remote_message(self, target: str, sender: str, content: str):
        ...
        # Standard IRC format: :Sender PRIVMSG Target :Message
        formatted_msg = f":{sender} PRIVMSG {target} :{content}"
        
        with self.users_lock:
            # Case 1: Target is a Channel (starts with #)
            ...
            # Case 2: Target is a User (Private Message)
            else:
    ...
    def _handle_client(self, sock, addr):
        ...
        #log.info(f"Connection accepted from {addr[0]}:{addr[1]}. Assigned temporary nick: {client.nick}")
    ...
    def handle_command(self, client: ClientRegistration, cmd: Command):
        if cmd.command == "NICK":
            ...
            if channel not in client.channels:
                ...
                self.send_reply(client,"353",f":{client.id()} JOIN {channel}")
                self.send_reply(client, "332",f" {client.nick} {channel} :{topic}")
            ...
            # 1. Always push to DynamoDB (so it's recorded and other servers see it)
            self.db.send_message(target, client.nick, msg)
            
            # 2. Optimization: If target is local, send immediately (don't wait for Poller)
            # The Poller filters out messages from 'self.server_id', so no duplicates.
            with self.users_lock:
                if target.startswith("#"):
                    # Channel Logic
                    local_recipients = [
                    ...
                    if target in self.users:
                        ...
                        # User is remote (on another EC2 instance)
                        # We already pushed to DynamoDB above.
                        # The OTHER server will poll, find it, and deliver it.
                        pass
            ...
            # For simplicity, we handle only the first channel in subcommands
            channel_name = cmd.subcommands[0]
            # Use the content as the parting message, default to nick if no content
            part_message = cmd.content if cmd.content else client.nick

            # 1. Check if the user is actually in the channel
            ...
            # 2. Protocol Message (Broadcast PART)
            part_msg = f":{client.id()} PART {channel_name} :{part_message}"
            
            # Find all local users in the channel (including the parting client)
            with self.users_lock:
                recipients = [
                    c for c in self.clients.values() 
            ...
            self.db.send_message(target, client.nick, msg)
            # 3. Server State Update (Local and DB)
            # Remove channel from local state
            client.channels.remove(channel_name)
                
            # Remove member from DynamoDB (this uses your existing, correct implementation)
            self.db.part_channel(client, channel_name)
        elif cmd.command == "LIST":
            if not client.registered:
                # 451 ERR_NOTREGISTERED: Client must be registered to use commands
                self.send_reply(client, "451", ":You have not registered", "LIST")
                return

            # 1. Send the Start of List marker (RPL_LISTSTART)
            # 321 <nick> Channel :Users Name
            self.send_reply(client, "321", "Channel :Users Name", target="Channel")

            # 2. Get all channels from DynamoDB
            channels = self.db.get_all_channels()

            # 3. Iterate and send RPL_LIST (322) for each channel
            for channel in channels:
                ...
                # Note: The target here is the channel name for the numeric reply
                self.send_reply(client, "322", f"{channel_name} {member_count} :{topic}", target=channel_name)

            # 4. Send the End of List marker (RPL_LISTEND)
...
if __name__ == "__main__":
    ...
    # Ensure this matches the region of your DynamoDB tables
